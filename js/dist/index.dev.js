...
var text = "";
var textArr = ["developer", "dog lover", "LOTR Fan"];
...
function addLetter() {
  ...
  if (letterIndex < text.length) {
    ...
    setTimeout(function () {
      // add letter
      textAnimated.textContent += text[letterIndex]; // call itself 
...
function removeLetter() {
  ...
  if (letterIndex >= 0) {
    ...
    setTimeout(function () {
      // remove letter
      textAnimated.textContent = text.slice(0, letterIndex); // call itself 
...
function updateText() {
  ...
  currentTextIndex++; //go to the first string index when currentTextIndex has reached the last one
  ...
  text = textArr[currentTextIndex]; //call addLetter and get the animation going

  addLetter();
} //the initial call to start everything


updateText(); // const text = ['cyb', 'rafa', 'dev'];
// const speed = 300;
